...
def compute_class_metrics(true, pred, l, basic_args=None):
        ...
        # 获取二分类混淆矩阵的4个值: TN, FP, FN, TP
        cm = confusion_matrix(binary_true, binary_pred, labels=[False, True])
        if cm.shape != (2, 2):  # 只出现一个类别时可能变成 (1,1)
            tn, fp, fn, tp = 0, 0, 0, 0
            if binary_true.sum() == 0:  # 全是负类
                tn = cm[0][0]
                fp = cm[0][1] if cm.shape[1] > 1 else 0
            elif binary_true.sum() == len(binary_true):  # 全是正类
                tp = cm[0][0]
...
class MultiClassMetrics:
    def __init__(self, basic_args, pred, prob, true):
        ...
        self.accuracy = accuracy_score(true, pred)
