...
from model.NeuroGPT.encoder.base import EEGModuleMixin, deprecated_args
    ...
    def __init__(
        ...
        if self.is_decoding_mode:
            ...
            self.fc = _FullyConnected(
                final_fc_length=final_fc_length, chunk_num=chunk_num,)
...
class _FullyConnected(nn.Module):
    def __init__(self, final_fc_length,
                 ...
                 hidden_channels=32, chunk_num=2):
        ...
        self.fc = nn.Sequential(
            nn.Linear(final_fc_length*chunk_num, out_channels),
            nn.ELU(),
    ...
    def forward(self, x):
        x = x.contiguous().view(x.size(0)//self.chunk_num, -1)
