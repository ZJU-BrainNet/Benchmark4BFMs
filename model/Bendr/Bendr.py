...
from model.Bendr.dn3.trainable.models import Classifier
from model.Bendr.dn3.trainable.layers import Permute
from model.Bendr.dn3.utils import DN3ConfigException
...
class Bendr_Trainer:
    ...
    def optimizer(args, model, clsf):
        return torch.optim.Adam(model.parameters(), args.model_lr, weight_decay=0.01)
...
class BENDR(Classifier):
    ...
    def __init__(self, args: Namespace, encoder_h=512, contextualizer_hidden=3076, projection_head=False,
                 new_projection_layers=0, dropout=0., trial_embeddings=None, layer_drop=0, mask_p_t=0.01, 
                 mask_p_c=0.005, mask_t_span=0.1, mask_c_span=0.1, multi_gpu=False):
        
        self.encoder_h = encoder_h
        self.contextualizer_hidden = contextualizer_hidden
        super().__init__(args.n_class, args.seq_len, args.cnn_in_channels)

        encoder = ConvEncoderBENDR(args.cnn_in_channels, encoder_h=encoder_h, dropout=dropout, projection_head=projection_head)
        encoded_samples = encoder.downsampling_factor(args.seq_len)
        ...
        tqdm.tqdm.write(encoder.description(sequence_len=args.seq_len))
        ...
        self.load_pretrained_modules(args)
        
        
    def load_encoder(self, args, encoder_file, freeze=True, strict=True):
        self.encoder.load(args, encoder_file, strict=strict)
        self.encoder.freeze_features(unfreeze=not freeze)

    def load_contextualizer(self, args, contextualizer_file, freeze=True, strict=True):
        self.contextualizer.load(args, contextualizer_file, strict=strict)
        self.contextualizer.freeze_features(unfreeze=not freeze)

    def load_pretrained_modules(self, args, freeze_encoder=True, freeze_contextualizer=True, 
                                freeze_position_conv=False, freeze_mask_replacement=True, strict=False):
        encoder_file = ModelPathArgs.Bendr_path
        contextualizer_file = ModelPathArgs.Bendr_contextualizer_path
        self.load_encoder(args, encoder_file, freeze=freeze_encoder, strict=strict)
        self.load_contextualizer(args, contextualizer_file, freeze=freeze_contextualizer, strict=strict)
...
class _BENDREncoder(nn.Module):
    ...
    def load(self, args, filename, strict=True):
        state_dict = torch.load(filename, map_location=f'cuda:{args.gpu_id}')
