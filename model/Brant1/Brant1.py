...
class Brant1(nn.Module):
    ...
    def load_pretrained_weights(args, scale):
        ...
        if args.load_pretrained:
            ...
            t_state_dict  = torch.load(os.path.join(ModelPathArgs.Brant1_root_path, dir_name, f'encoder_ckpt/encoder_t_{args.start_epo_idx}.pt'),
                                       map_location=map_location)
            ch_state_dict = torch.load(os.path.join(ModelPathArgs.Brant1_root_path, dir_name, f'encoder_ckpt/encoder_ch_{args.start_epo_idx}.pt'),
                                       map_location=map_location)
